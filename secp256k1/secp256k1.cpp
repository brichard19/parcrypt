...
static std::default_random_engine _rng(_r());
static std::uniform_int_distribution<unsigned int> _dist;

static void get_random_bytes(unsigned char* buf, int count)
{
  for(int i = 0; i < count; i++) {
    buf[i] = (unsigned char)_dist(_rng);
  }
}
...
uint256 uint256::powModP(const uint256 x) const
{
  uint256 n = x;
  uint256 product(1);
  uint256 square = *this;
...
uint256 uint256::sqrtModP() const
{
...
uint256 secp256k1::fillBits(int bits)
{
  uint256 k;
...
std::string secp256k1::uint256::to_string(int base) const
{
  std::string s = "";
  if(base == 16) {
    for(int i = 7; i >= 0; i--) {
    ...
    secp256k1::uint256 t = *this;


    while(!t.isZero()) {
      secp256k1::uint256 k1 = t.div(10);
      secp256k1::uint256 k2 = k1.mul(10);
      secp256k1::uint256 diff = t - k2;
      s = std::to_string(diff.v[0]) + s;
...
uint256 secp256k1::generatePrivateKey()
{
  ...
  get_random_bytes((unsigned char*)k.v, 32);

  return k;
}

uint256 secp256k1::generatePrivateKey(unsigned int bits)
{
  uint256 k;

  get_random_bytes((unsigned char*)k.v, (bits + 7) / 8);
  k.v[bits / 32] &= (0xffffffff >> (32 - (bits % 32)));
...
secp256k1::uint256 secp256k1::parsebase10(std::string s)
{
  secp256k1::uint256 result(0);

  for(int i = 0; i < s.length(); i++) {
    result = result.add((int)(s[i] - '0'));
...
ecpoint secp256k1::negPoint(const ecpoint& p)
{
  secp256k1::ecpoint point = p;
  point.y = secp256k1::negModP(point.y);
...
void secp256k1::generateKeyPairsBulk(unsigned int count, const ecpoint& basePoint, std::vector<uint256>& privKeysOut, std::vector<ecpoint>& pubKeysOut, int keylen)
{
  privKeysOut.clear();

  for(unsigned int i = 0; i < count; i++) {
    privKeysOut.push_back(generatePrivateKey(keylen));
  }
...
secp256k1::ecpoint parse_uncompressed(const std::string& pubKeyString)
{
...
secp256k1::ecpoint parse_compressed(const std::string& pubKeyString)
{
  if(pubKeyString.substr(0, 2) != "02" && pubKeyString.substr(0, 2) != "03") {
    throw std::string("Invalid public key");
  }

  if(pubKeyString.length() != 66) {
    throw std::string("Invalid public key");
  }

  unsigned char sign;
  if(pubKeyString[0] == '0' && pubKeyString[1] == '2') {
    sign = 0x02;
  } else {
    sign = 0x03;
  }

  std::string xString = pubKeyString.substr(2);

  secp256k1::uint256 x(xString);

  secp256k1::ecpoint p = secp256k1::ecpoint(x, sign);
...
{
  std::string prefix = pubKeyString.substr(0, 2);

  if(prefix == "04") {
    return parse_uncompressed(pubKeyString);
  } else if(prefix == "02" || prefix == "03") {
    return parse_compressed(pubKeyString);
  } else {
    throw std::string("Invalid public key");
  }
}

bool ecpoint::exists() const
{
  return secp256k1::pointExists(*this);
}